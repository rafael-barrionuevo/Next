...
function Carousel({ items = [], autoplay = false }) {
  ...
  const swiperModules = autoplay ? [Pagination, Autoplay] : [Pagination];
        ...
        >
          {items.map((item, index) => (
            <SwiperSlide key={item._id || index} className='overflow-visible'>
              <div
                className='relative aspect-video bg-zinc-900 rounded-2xl overflow-hidden group border border-white/5 hover:border-purple-600/50 shadow-lg hover:shadow-purple-600/20 transition-all duration-500 cursor-pointer'
              ...
              >
                ...
                />
                {/* Gradient overlay with blur for better text contrast */}
                <div className='absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-90 group-hover:opacity-100 group-hover:backdrop-blur-[2px] transition-all duration-300 z-10 pointer-events-none' />
